...
test("dashboard staff enlaza conteos y aplica filtros allowlisted en servidor", async ({ page }) => {
  ...
  const inserted = await admin.from("listings").insert([
    ...
    { owner_id: normalUser.id, title: "Aprobado legado dashboard", status: "approved", submitted_at: now },
  ...
  const approvedId = byStatus("approved")[0].id;
  ...
  try {
    ...
    await expect(listingLink(approvedId)).toHaveCount(0);

    await openView("legacy-approved", "Aprobados pendientes de publicar");
    await expect(listingLink(approvedId)).toHaveCount(1);
    await expect(listingLink(publishedId)).toHaveCount(0);
    ...
    const keyboardCards = [
      ...
      ["legacy-approved", "Aprobados pendientes de publicar"],
    ] as const;
...
test("usuario normal no abre vistas filtradas de staff", async ({ page }) => {
  ...
  try {
    ...
    for (const view of ["pending", "in-review", "mine", "changes-requested", "published", "legacy-approved"]) {
