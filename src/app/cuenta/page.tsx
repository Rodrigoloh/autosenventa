...
export default async function AccountPage() {
  ...
  const { data: updatesData } = await supabase
    ...
    .in("status", ["changes_requested", "approved", "rejected"])
          ...
          {updates.map((listing) => {
            ...
            return (
              <article key={listing.id} className="py-5">
                <p className="font-black">{listing.status === "changes_requested" ? "Tu anuncio requiere cambios." : listing.status === "approved" ? "Tu anuncio fue aprobado." : "Tu anuncio fue rechazado."}</p>
                <p className="mt-1 text-sm font-semibold text-stone-700">{listing.title}</p>
                {listing.status === "approved" ? <p className="mt-2 text-sm text-stone-600">La aprobación no significa que ya esté publicado.</p> : null}
                {decisionDate ? <p className="mt-2 text-xs text-stone-500">Decisión del {formatDate(decisionDate)}</p> : null}
                <Link href={`/cuenta/anuncios/${listing.id}/${listing.status === "changes_requested" ? "editar" : "vista-previa"}`} className="mt-3 inline-flex min-h-11 items-center font-bold underline">
                  {listing.status === "changes_requested" ? "Editar y corregir" : listing.status === "approved" ? "Ver anuncio" : "Ver motivo"}
                </Link>
              </article>
            );
          })}
