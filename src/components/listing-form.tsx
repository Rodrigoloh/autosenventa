...
import { saveListingAction } from "@/app/cuenta/anuncios/actions";
...
export function ListingForm({ listing, categories, brands, models }: {
  ...
  return (
    <form action={formAction} onReset={(event) => event.preventDefault()} className="mt-10 space-y-10" noValidate>
      <Section title="Identificación" description="Los catálogos activos provienen de la taxonomía editorial.">
        <label className="block text-sm font-semibold">
          Categoría
          <select className={inputClass} name="category_id" defaultValue={listing.category_id ?? ""} aria-invalid={errors.category_id?.length ? true : undefined}>
        ...
        <label className="block text-sm font-semibold">
          Marca
          <select className={inputClass} name="brand_id" value={brandId} onChange={(event) => {
        ...
        <label className="block text-sm font-semibold">
          Modelo
          <select className={inputClass} name="model_id" value={modelId} onChange={(event) => setModelId(event.target.value)} disabled={!brandId} aria-invalid={errors.model_id?.length ? true : undefined}>
            <option value="">{brandId ? "Selecciona un modelo" : "Selecciona primero una marca"}</option>
