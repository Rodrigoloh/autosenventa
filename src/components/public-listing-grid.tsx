...
export function PublicListingGrid({ listings }: { listings: PublicListingSummary[] }) {
  return <div data-public-listing-grid className="grid min-w-0 gap-x-4 gap-y-9 sm:grid-cols-2 lg:grid-cols-4">{listings.map((listing) => <PublicListingCard key={listing.id} listing={listing} />)}</div>;
}
